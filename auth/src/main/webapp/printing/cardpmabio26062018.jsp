...
<!-- 				<br /> -->
				...
				<c:forEach items="${stavkart}" var="stavka">
					...
					<c:if test="${(aktivniArtikal !=stavka.artikli.id || aktivniMagacin!=stavka.idDokument.magacini.id || aktivniPartner!=stavka.idDokument.partner.id) && firstTime==false}">
...
// 				long s = Long.parseLong(dic[1]);
				...
				%>
						...
						<!-- ************************* KRAJ UKUPNO ZA ARTIKAL ********************************  -->
								...
								<c:if test="${(aktivniMagacin!=stavka.idDokument.magacini.id || aktivniPartner!=stavka.idDokument.partner.id) && firstTime==false}">
									<%-- <c:out value="artikli nisu isti" /> --%>
							</tbody>
						</table>
						<table class='tableDocuments'>
					...
					<c:if test="${aktivniPartner!=stavka.idDokument.partner.id  && firstTime==false}">
						<%-- <c:out value="artikli nisu isti" /> --%>
						</tbody>
						</table>
						<table class='tableDocuments'>
					...
					<c:set var="totalDugujes" value="${totalDugujes + stavka.duguje}" />
...
%>
    <table>
      <TH>Name</th>
      <TH>Id</th>
      <c:forEach items="${sk}" var="current">
        <tr>
          <td><c:out value="${curent.IdArtikal}" /><td>
          <td><c:out value="${sk.current.IdMagacin}" /><td>
        </tr>
      </c:forEach>
    </table>
			</div>
