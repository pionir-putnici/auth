						...
						<div class='panel-body'>
							<div class='row'>
								<div class='col-lg-6'>
									...
									<form:form method="POST" action="save_states.html"
										...
										<div class='form-group ${error != null ? 'has-error' : ''}'>
											...
											<form:input type="text" class="form-control " name="code"
												path="code" value="${states.code}" autofocus="true"
										...
										<div class='form-group'>
											...
											<form:textarea type="text" class="form-control" id="remark"
												path="remark" value="${states.remark}" />
											<p class="help-block">
												<form:errors path="remark" class="help-block" />
											</p>
										</div>

										<c:out value="${model.states.aktivan}" />

										<c:if test="${states.aktivan}">
											<c:out value="${states.code}" />
										</c:if>
										...
										<c:if test="${states.aktivan == true}">
											<c:set var="salary" scope="session" value="checked" />
											<c:set var="salary1" scope="session" value="" />
										</c:if>

										<c:if test="${states.aktivan == false}">
