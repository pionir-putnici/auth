						...
						<div class='panel-body'>
							<div class='row'>
								<div class='col-lg-6'>
									...
									<form:form method="POST" action="save_units_of_measure.html"
										modelAttribute="jediniceMera">

										<input type="hidden" name="${_csrf.parameterName}"
											value="${_csrf.token}" />

										<form:input type="hidden" name="id" path="id"
											value="${jediniceMera.id}" />

										<div class='form-group ${error != null ? 'has-error' : ''}'>
											<label class='font-oxygen'>Šifra</label>

											<form:input type="text" class="form-control " name="code"
												path="code" value="${jediniceMera.code}" autofocus="true"
										...
										<div class='form-group'>
											...
											<form:textarea type="text" class="form-control" id="remark"
												path="remark" value="${jediniceMera.remark}" />
											<p class="help-block">
												<form:errors path="remark" class="help-block" />
											</p>
										</div>

										<c:out value="${model.jediniceMera.aktivan}" />

										<c:if test="${jediniceMera.aktivan}">
											<c:out value="${jediniceMera.code}" />
										</c:if>
										...
										<c:if test="${jediniceMera.aktivan == true}">
											<c:set var="salary" scope="session" value="checked" />
											<c:set var="salary1" scope="session" value="" />
										</c:if>

										<c:if test="${jediniceMera.aktivan == false}">
