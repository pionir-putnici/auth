					...
					<div class='panel panel-default'>
						...
						<div class='panel-body'>
							<div class='row'>
								...
								<form:form method="POST" action="save-vrste-magacina.html"
										modelAttribute="vrsteMagacina">

										<input type="hidden" name="${_csrf.parameterName}"
											value="${_csrf.token}" />

										<form:input type="hidden" name="id" path="id"
											value="${vrsteMagacina.id}" />

										<div class='form-group ${error != null ? 'has-error' : ''}'>
											<label class='font-oxygen'>Šifra</label>

											<form:input type="text" class="form-control " name="code"
												path="code" value="${vrsteMagacina.code}" autofocus="true"
										...
										<div class='form-group'>
											...
											<form:textarea type="text" class="form-control" id="remark"
												path="remark" value="${vrsteMagacina.remark}" />
											<p class="help-block">
												<form:errors path="remark" class="help-block" />
											</p>
										</div>

										<c:out value="${model.vrsteMagacina.aktivan}" />

										<c:if test="${vrsteMagacina.aktivan}">
											<c:out value="${vrsteMagacina.code}" />
										</c:if>
										...
										<c:if test="${vrsteMagacina.aktivan == true}">
											<c:set var="salary" scope="session" value="checked" />
											<c:set var="salary1" scope="session" value="" />
										</c:if>

										<c:if test="${vrsteMagacina.aktivan == false}">
										...
										<a href="/u-vrste-magacina.html">Visit W3Schools</a> 
